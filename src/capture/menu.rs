...
pub fn render_elements(
    ...
    elements.extend(crate::backend::border_strips(bar, 2, highlight));
    ...
    for (index, item) in layout.items.into_iter().enumerate() {
        ...
        elements.extend(crate::backend::border_strips(item, 2, accent));
...
fn push_icon(
    ...
    match index {
        ...
        1 => {
            elements.extend(crate::backend::border_strips(
        ...
        _ => {
            elements.extend(crate::backend::border_strips(icon, line, color));
            add(
                elements,
                Rectangle::new(
